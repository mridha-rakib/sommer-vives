...
-- Function to generate standard sag tasks for a listing
CREATE OR REPLACE FUNCTION public.generate_sag_tasks(p_listing_id uuid, p_listing_name text, p_assigned_to uuid DEFAULT NULL, p_assigned_name text DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  task_list text[][] := ARRAY[
    -- Phase 1: Opstart & Formalia (10 tasks)
  ...
  i int;
BEGIN
  -- Don't create if tasks already exist for this listing
...
-- Trigger function for auto-creating tasks on new listings
CREATE OR REPLACE FUNCTION public.auto_generate_sag_tasks()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
BEGIN
  PERFORM generate_sag_tasks(NEW.id, NEW.name);
  RETURN NEW;
END;
$$;

-- Create trigger on listings table
DROP TRIGGER IF EXISTS trg_auto_sag_tasks ON public.listings;
CREATE TRIGGER trg_auto_sag_tasks
  AFTER INSERT ON public.listings
  FOR EACH ROW
  EXECUTE FUNCTION auto_generate_sag_tasks();

-- Seed tasks for existing 3 listings
SELECT generate_sag_tasks(id, name) FROM listings;
