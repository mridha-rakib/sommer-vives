...
const ReviewCard = ({ review, index, locale }: { review: Review; index: number; locale: string }) => {
  const dateLabel = getDateLabel(review.date, locale);
...
const FeaturedCarousel = ({ reviews, copy }: { reviews: Review[]; copy: typeof reviewCopy.da }) => {
  ...
  const dateLabel = getDateLabel(review.date, copy.locale);

  return (
    <div className="text-center max-w-3xl mx-auto py-8 relative">
      <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-16 w-10 h-10 rounded-full border border-border/30 flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors" aria-label={copy.previous}>
        <ChevronLeft className="h-4 w-4" />
      </button>
      <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-16 w-10 h-10 rounded-full border border-border/30 flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors" aria-label={copy.next}>
...
export const ReviewsSection = ({
  ...
  const copy = language === 'en' ? reviewCopy.en : reviewCopy.da;
  const reviews = providedReviews?.length ? providedReviews : sampleReviews;
  const averageRating = rating || (reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length);
  ...
  return (
    <section className="py-8 lg:py-12">
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          <span className="text-primary font-display italic text-sm mb-2 block">{copy.eyebrow}</span>
          ...
          <div className="flex items-center justify-center gap-5 mt-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-display text-xl font-semibold text-foreground">{averageRating.toFixed(2)}</span>
              <span className="text-sm text-muted-foreground">/ 5</span>
            </div>
            <span className="w-px h-5 bg-border" />
            <span className="text-sm text-muted-foreground">{count || reviews.length} {copy.reviews}</span>
          </div>
        </motion.div>

        {variant === 'featured' ? (
          <FeaturedCarousel reviews={reviews} copy={copy} />
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-12">
            {reviews.map((r, i) => (
              <ReviewCard key={r.id} review={r} index={i} locale={copy.locale} />
            ))}
          </div>
        )}
      </div>
