        ...
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
            <h2 className="font-display text-4xl md:text-6xl lg:text-7xl font-bold mb-6 leading-[0.95]">
              {t('contact.title')}{' '}
              <span className="block text-primary italic font-normal mt-2">{t('contact.titleAccent')}</span>
            </h2>
            <p className="text-lg md:text-xl text-muted-foreground leading-relaxed mb-8 max-w-lg mx-auto">{t('contact.subtitle')}</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.5, delay: 0.2 }}
            ...
            <span className="text-primary text-sm font-semibold">{t('contact.urgency')}</span>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }}
            ...
            <Link to="/kom-i-gang">
              <Button variant="gold" size="xl" className="gap-3 group text-base w-full sm:w-auto rounded-full">
                {t('contact.cta1')}<ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Button>
            </Link>
            <Link to="/book-vurdering">
              <Button variant="outline" size="xl" className="border-border text-muted-foreground hover:bg-muted hover:text-foreground text-base w-full sm:w-auto rounded-full">
                {t('contact.cta2')}
              </Button>
            </Link>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.6 }}
            ...
            <span className="w-1 h-1 rounded-full bg-muted-foreground/40" />
            <span className="text-sm">{t('contact.area')}</span>
          </motion.div>
        </div>
      </div>
    </section>
