...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-[100dvh] flex flex-col overflow-hidden">
      ...
      >
        {/* Gradient background */}
        <div className="absolute inset-0 bg-gradient-to-r from-accent/15 via-accent/8 to-accent/15 backdrop-blur-md" />
        ...
        <div className="relative container mx-auto px-4 py-4 sm:py-5">
          ...
          >
            <div className="flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-accent/20 border border-accent/30">
              <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-accent" />
            </div>
            <div>
              <p className="text-[10px] sm:text-xs font-bold text-accent tracking-[0.15em] uppercase">
                Merindtjening kun hos os
              </p>
              <p className="text-[9px] sm:text-[10px] text-primary-foreground/50 font-medium">
                Tjen penge på alt — ikke kun overnatningen
              </p>
            </div>
          </motion.div>

          {/* Revenue items — scrollable on mobile */}
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {[
              { icon: Zap, label: 'El', desc: 'Forbrug' },
              { icon: Droplets, label: 'Vand', desc: 'Forbrug' },
              { icon: Flame, label: 'Varme', desc: 'Forbrug' },
              { icon: BedDouble, label: 'Sengepakker', desc: 'Tillæg' },
              { icon: Shirt, label: 'Håndklæder', desc: 'Tillæg' },
              { icon: Clock, label: 'Tidlig check-in', desc: 'Ekstra' },
              { icon: Clock, label: 'Sen check-out', desc: 'Ekstra' },
              { icon: Dog, label: 'Husdyr', desc: 'Tillæg' },
              { icon: UtensilsCrossed, label: 'Slutrengøring', desc: 'Gebyr' },
              { icon: Wifi, label: 'Premium WiFi', desc: 'Tillæg' },
              { icon: TreePine, label: 'Brænde', desc: 'Forbrug' },
              { icon: ShieldCheck, label: 'Skadesforsikring', desc: 'Gebyr' },
              { icon: Sparkles, label: 'Meget mere', desc: '+' },
            ].map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.35, delay: 1.5 + i * 0.06 }}
                className="flex-shrink-0 flex items-center gap-2 bg-primary-foreground/8 hover:bg-accent/15 backdrop-blur-sm rounded-xl px-3 py-2 border border-primary-foreground/8 hover:border-accent/25 transition-all duration-300 group cursor-default"
              >
                <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-accent/15 group-hover:bg-accent/25 transition-colors">
                  <item.icon className="w-3.5 h-3.5 text-accent group-hover:scale-110 transition-transform" />
                </div>
                <div className="flex flex-col">
                  <span className="text-[11px] sm:text-xs text-primary-foreground/90 font-semibold leading-tight whitespace-nowrap">{item.label}</span>
                  <span className="text-[9px] text-primary-foreground/40 font-medium leading-tight">{item.desc}</span>
                </div>
              </motion.div>
            ))}
          </div>
