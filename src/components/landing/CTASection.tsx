import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { ArrowRight, Calculator } from 'lucide-react';

export function CTASection() {
  return (
    <section className="section-padding bg-primary">
      <div className="container mx-auto text-center">
        <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-background mb-6">
          Klar til at udleje dit sommerhus?
        </h2>
        <p className="text-lg md:text-xl text-background/80 mb-10 max-w-2xl mx-auto">
          Kom i gang i dag og lad os tage os af resten – fra markedsføring til gæstekontakt.
        </p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link to="/kom-i-gang">
            <Button size="xl" className="bg-background text-foreground font-semibold hover:bg-background/90 shadow-elevated text-lg px-8 py-6">
              Opret dit sommerhus gratis
              <ArrowRight className="w-5 h-5" />
            </Button>
          </Link>
          <Link to="/beregn-lejeindtaegt">
            <Button variant="outline" size="xl" className="border-background/30 text-background hover:bg-background/10">
              <Calculator className="w-5 h-5" />
              Beregn din indtægt
            </Button>
          </Link>
        </div>
        <p className="mt-6 text-sm text-background/60">
          6 måneders binding • Ingen oprettelsesgebyr • Kun 15% kommission
        </p>
      </div>
    </section>
  );
}
