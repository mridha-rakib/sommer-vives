...
import {
  ...
  Bell, Target, UserCheck, Bed, X, Trash2
} from 'lucide-react';
...
const EVENT_CFG: Record<EventType, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  lead_followup: { label: 'Lead opfølgning',color: 'text-orange-400',  bg: 'bg-orange-500/15',  icon: Target },
  checkin:       { label: 'Check-in',       color: 'text-emerald-400', bg: 'bg-emerald-500/15', icon: UserCheck },
  checkout:      { label: 'Check-out',      color: 'text-rose-400',    bg: 'bg-rose-500/15',    icon: Bed },
};
...
function mapSystemTask(t: any): CalEvent | null {
  if (!t.due_date) return null;
  const dateStr = typeof t.due_date === 'string' ? t.due_date.split('T')[0] : '';
  if (!dateStr) return null;
  const timePart = t.due_date.includes('T') ? t.due_date.split('T')[1]?.substring(0, 5) : undefined;

  let type: EventType = 'task';
  if (t.linked_type === 'meeting' || t.title?.startsWith('📅')) type = 'meeting';
  else if (t.priority === 'urgent') type = 'reminder';

  return {
    id: `st-${t.id}`, type, title: t.title?.replace(/^📅\s*/, '') || 'Opgave',
    date: dateStr, time: timePart && timePart !== '00:00' ? timePart : undefined,
    linked: t.linked_name || undefined, source: 'system_task', sourceId: t.id,
  };
}
...
function mapLead(l: any): CalEvent | null {
  ...
  return {
    ...
    title: l.next_step || `Opfølgning: ${l.name}`,
...
function mapBooking(b: any, type: 'checkin' | 'checkout'): CalEvent {
  ...
  return {
    ...
    title: `${type === 'checkin' ? 'Check-in' : 'Check-out'}: ${b.guest_name || 'Gæst'}`,
...
export default function AdminKalender() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // System tasks (meetings, opgaver)
    ((stRes.data as any[]) || []).forEach(t => {
      const evt = mapSystemTask(t);
  ...
  const handleDelete = async (evt: CalEvent) => {
    if (evt.source === 'system_task') {
      if (!confirm('Slet denne begivenhed?')) return;
      await supabase.from('system_tasks' as any).delete().eq('id', evt.sourceId);
      toast.success('Slettet');
      setDetailEvent(null);
      fetchAll();
    } else if (evt.source === 'lead') {
      if (!confirm('Fjern opfølgningsdato fra dette lead?')) return;
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <AdminPageHeader
          title="Kalender"
          subtitle="Møder, lead-opfølgninger, check-ins og opgaver"
          actions={
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-semibold text-foreground min-w-[180px] text-center">
              ...
              {view === 'week' && `Uge — ${MONTHS[month]} ${year}`}
                ...
                return (
                  <div key={dateStr} className="border-r border-border/10 last:border-r-0">
                    ...
                    <div className="p-2 min-h-[300px] space-y-1.5">
                      ...
                      {dayEvts.length === 0 && <p className="text-[10px] text-muted-foreground/30 text-center pt-8">Ingen</p>}
                    </div>
                  </div>
                );
              })}
            ...
            return (
              <>
                ...
                <div className="space-y-3 py-2">
                  ...
                  <div className="rounded-lg border border-border/30 bg-muted/10 p-3 space-y-2">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium text-foreground">{new Date(detailEvent.date).toLocaleDateString('da-DK', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </div>
                    {detailEvent.linked && (
                      <div className="flex justify-between text-xs">
                        <span className="text-muted-foreground">Tilknyttet</span>
                        <span className="font-medium text-foreground">{detailEvent.linked}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-xs">
                      <span className="text-muted-foreground">Kilde</span>
                      <span className="font-medium text-foreground capitalize">{detailEvent.source === 'system_task' ? 'Opgave' : detailEvent.source === 'lead' ? 'Lead' : 'Booking'}</span>
                    </div>
                  </div>
                </div>
                <DialogFooter className="gap-2">
                  ...
                  <Button variant="outline" size="sm" className="rounded-xl text-xs" onClick={() => setDetailEvent(null)}>Luk</Button>
                </DialogFooter>
              </>
            );
          })()}
...
function CreateCalendarEventDialog({ open, onClose, onCreated, defaultDate }: { open: boolean; onClose: () => void; onCreated: () => void; defaultDate: string }) {
  ...
  const save = async () => {
    ...
    if (eventType === 'lead_followup') {
      // Find or inform — we create a system_task with lead link
      ...
      const { error } = await supabase.from('system_tasks' as any).insert({
        title: eventType === 'meeting' ? `📅 ${title.trim()}` : title.trim(),
        description: notes.trim() || null,
        priority: 'normal',
        linked_type: eventType === 'meeting' ? 'meeting' : null,
        linked_name: linkedName.trim() || null,
        due_date: dueDate,
        assigned_to: user?.id || null,
        created_by: user?.id || null,
  ...
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Plus className="h-4 w-4 text-primary" />Tilføj til kalender</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div>
            ...
            <Select value={eventType} onValueChange={v => setEventType(v as any)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="lead_followup">Lead opfølgning</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
            ...
            <Input value={title} onChange={e => setTitle(e.target.value)} className="mt-1 rounded-xl"
              placeholder={eventType === 'meeting' ? 'Fremvisning sommerhus' : eventType === 'lead_followup' ? 'Ring til lead' : 'Hvad skal gøres?'} />
          </div>
          ...
          <div>
            <Label className="text-xs">{eventType === 'lead_followup' ? 'Lead-navn' : 'Kontaktperson / tilknytning'}</Label>
            <Input value={linkedName} onChange={e => setLinkedName(e.target.value)} className="mt-1 rounded-xl" placeholder="Navn" />
          </div>
          <div>
        ...
        <DialogFooter>
          ...
          <Button onClick={save} disabled={saving} className="rounded-xl gap-1.5">
            <Plus className="h-3.5 w-3.5" />{saving ? 'Opretter...' : 'Tilføj'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
