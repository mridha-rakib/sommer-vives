      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="flex items-center gap-2 flex-wrap flex-1">
            ...
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value as any)} className="h-8 text-xs rounded-lg border border-border bg-background px-2"><option value="all">Alle statusser</option>{STATUSES.map(s => <option key={s} value={s}>{STATUS_CFG[s].label}</option>)}</select>
            <select value={filterPriority} onChange={e => setFilterPriority(e.target.value as any)} className="h-8 text-xs rounded-lg border border-border bg-background px-2"><option value="all">Alle prioriteter</option>{PRIORITIES.map(p => <option key={p} value={p}>{PRIORITY_CFG[p].label}</option>)}</select>
            <select value={filterLinked} onChange={e => setFilterLinked(e.target.value)} className="h-8 text-xs rounded-lg border border-border bg-background px-2"><option value="all">Alle typer</option>{LINKED_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}</select>
